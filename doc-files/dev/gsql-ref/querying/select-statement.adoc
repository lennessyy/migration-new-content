...
{% hint style="success" %}
New Feature in v3.1: SQL-like tabular SELECT and output. +
See link:select-statement.md#sql-like-select-statement[SQL-Like SELECT Statement]
{% endhint %}
...
{% hint style="success" %}
The optional SQL-like SELECT statement defines a tabular output. +
It introduces the link:select-statement.md#group-by-clause[GROUP BY clause], as well as internal table structures which can be printed.
{% endhint %}
...
The SQL-like form may use the link:select-statement.md#group-by-clause[GROUP  BY clause] but may not use the ACCUM and POST-ACCUM clauses.
...
Run this query on the graph `socialNet`, and the results would be:
...
In classic (Syntax v1) GSQL, the FROM clause described one step or hop pattern. In link:select-statement.md#pattern-matching-syntax-v-2[Pattern Matching] (Syntax v2), the pattern can be multiple hops long. Path patterns also have many other options for finer control and greater flexibility.
